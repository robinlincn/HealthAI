...
系统包含三大主要部分：**病人端 (手机App风格)**, **医生端 (PC管理后台)**, 和 **SAAS管理后台 (PC管理后台)**。
...
一个使用 Vue.js 3, Vite, Pinia, 和 Tailwind CSS 构建的病人端应用正在开发中。它旨在复制 Next.js 版本的大部分功能和风格。
- 目录: `vue-patient-app/`
- 主要技术: Vue 3, Vite, Vue Router, Pinia, Tailwind CSS, Lucide-vue-next
- 当前状态: 基础布局、导航和大部分主要页面的UI骨架已搭建，许多具体功能为占位符。通过 `/vue-patient-app/` 访问。
...
### 医生端核心功能 (Vue.js Version - In Development):
一个使用 Vue.js 3, Vite, Pinia, 和 Tailwind CSS 构建的医生端应用正在开发中。它旨在复制 Next.js 版本的大部分功能和风格。
- 目录: `vue-doctor-app/`
- 主要技术: Vue 3, Vite, Vue Router, Pinia, Tailwind CSS, Lucide-vue-next
- 当前状态: 基础布局、导航和主要页面的UI骨架已搭建，大部分具体功能为占位符。通过 `/vue-doctor-app/` 访问。
...
- **Vue.js 前端 (医生端 - 新增)**: Vue 3, Vite, TypeScript, Vue Router, Pinia
...
1.  **确保您没有未提交的本地更改**：
    ...
    *   如果有未提交的更改，您可以选择：
        *   暂存更改: `git stash push -u -m "我的本地修改"`
        *   提交更改: `git add .` 然后 `git commit -m "描述您的提交"`
...
5.  **处理可能的冲突**：
    *   如果在 `git pull` 过程中发生合并冲突，您需要手动解决这些冲突，然后 `git add <resolved-files>`，最后 `git commit`。

6.  **应用数据库迁移 (如果适用)**：
    *   如果 `db/schema.sql` 或 `db/database_schema.md` 有更新，您可能需要相应地更新您的本地数据库模式。
